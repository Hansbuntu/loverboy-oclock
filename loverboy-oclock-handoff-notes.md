...
- Phone layout + haptics (2026-09-20): on portrait phones the play-field is 500 wide x 400-440 tall (`fitCanvas()` in `js/game.js`; desktop/landscape stay 640x400) so sprites/pillars draw ~25% bigger. The game area is `clamp(80vw, 100svh - 318px, 88vw)` tall and the tracklist card flex-fills the rest, so the page is exactly one screen with no trailing whitespace. Haptics: `haptic(kind)` -> Vibration API on Android; on iPhone (no Vibration API) a hidden `<input switch>` is toggled for the system tick (iOS 17.4+ Safari; only verifiable on a real device).
- Premium upgrade, phase 1 (2026-09-26): fixed 60 Hz physics with render interpolation (same tuning, identical on 60/90/120 Hz); canvas drawn at device resolution (`SCALE`) so art is sharp on phones; "get ready" hover before the first tap; squash/stretch, kick frame (`fly-1.png` = jump-1 without its shadow) and heart puffs on flap; crash = freeze-frame + white flash + in-canvas shake + knock-back tumble + stone chips, then a small fail card (score, best per level in `loverboy_oclock_best_v1`, retry locked for 420 ms); near-miss "Close!" (< 9 px clearance); slow-mo glide + bell on the last gap; all SFX redesigned (pentatonic chime per gap); music routed through Web Audio (volume now works on iPhone, lowpass muffle under menus, tape-stop slow-down on a crash, resume on retry). Test on a real phone once the mp3s exist.
- Premium upgrade, phase 2 (2026-09-26): the world. Per-level weather from `fx` in `js/config.js` (1 mist+stars, 2 red petals + glowing gate crack, 3 rain + lightning + thunder, 4 fireflies + birds + warm haze, 5 embers + flickering red line, 6 dust motes + sun rays, 7 rotating sunbeams + gold petals); `fx` anchor points are fractions of the background image. Scrolling ground (`assets/ground/`) and a faster foreground silhouette strip for depth; vignette that lifts as the hours go by. Banner and torch pillars (standing only, widened to match the hitbox). HUD is now the hour ("3 O'Clock") plus an SVG clock whose minute hand sweeps once per level and strikes on the last gap. The character carries a heart that is locked and dim at 1 O'Clock, brighter each hour, and bursts open on the last gap of 7 O'Clock.
- Premium upgrade, phase 3 (2026-09-26): the screens. Intro screens show the world: he stands idle on a "launch" pillar (idle frames + `idle-blink.png`, a closed-eyes copy of idle-1) and arcs off it into the hover on the first tap. Title letters drop in and wave. The unlock card is a cassette (tile on the label, reels spin and the tape winds across as the preview's progress). Pixel-wipe transitions (reveal -> next level / finale, play again). The tracklist ("The tape") is a jukebox: tap an unlocked track to replay its preview (ducks the game music; shows "coming soon" if the mp3 is missing). Finale: rising pixel hearts, the assembled tape (7 tappable tiles), and "Share your tape" (pre-rendered 1080x1350 image via Web Share; desktop falls back to saving the image + copying the link). Link previews use `assets/share/og-image.jpg` (1200x630, rendered by the same `renderShareCard` code). `site.url` in config.js is the address used for sharing.
- Changes after phase 3 (2026-09-26): every attempt starts on the pedestal (title, each level, and after every fail) and the tap launches him off it; the mid-air hover is gone. Unlock previews always play 10 s (a shorter clip loops, minimum 7 s; `previewMs` / `previewMinMs` in config.js) with a countdown and no skipping. "Share your tape" was removed from the finale (the game is for players only); the link-preview image stays.
- Premium upgrade, phase 4 (2026-09-26): extras. Hearts: 3 per level float in the open air between pillars (30-70 px off the gap, never after the last pillar); best per level in `loverboy_oclock_hearts_v1`, shown in the HUD, on the cassette card, as mini hearts on each unlocked track and as a total on the finale. Tested: a bot ignoring them picks up ~8/21 by accident; one going for them gets ~17/21 without crashing. Quiet help: after 4/7/10 fails on a level (this visit) its gaps widen by 8/16/24 px and slides shorten; resets on clear. Pause: button during play, Esc/P, and automatic when the app is hidden; resume counts down 3-2-1; "Restart this hour". Sound and vibration toggles (pause menu + tape panel) saved in `loverboy_oclock_settings_v1`. Automatic low-quality mode if >50 of 180 frames are slow (fewer pixels, half the weather).
